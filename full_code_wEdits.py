...
while(not(convergence)):
    ...
    charPatchLParam = [0]*(size);           # mean characteristic patch length
    charPatchLParamVar = [0]*(size);        # mean characteristic patch length variance
    charPatchLVarParam = [0]*(size);        # characteristic patch length variance
    charPatchLVarParamVar = [0]*(size);     # characteristic patch length variance
    charPatchWParam = [0]*(size);           # mean patch width
    charPatchWParamVar = [0]*(size);        # mean patch width
    charPatchWVarParam = [0]*(size);        # patch width variance
    charPatchWVarParamVar = [0]*(size);     # characteristic patch length variance
    desiredCoverageParam = [0]*(size);      # fraction of sample covered by graphene
    desiredCoverageParamVar = [0]*(size);
    
    for i in range(size): #set the parameters for each layer using user input
        charPatchLParam[i] = float(dfinput.loc[i,'charLengthL']); 
        charPatchLParamVar[i] = float(dfinput.loc[i,'charLengthLVAR']);
        charPatchLVarParam[i] = float(dfinput.loc[i,'charLengthVarL']);
        charPatchLVarParamVar[i] = float(dfinput.loc[i,'charLengthVarLVAR']);
        charPatchWParam[i] = float(dfinput.loc[i,'charLengthW']);
        charPatchWParamVar[i] = float(dfinput.loc[i,'charLengthWVAR']);
        charPatchWVarParam[i] = float(dfinput.loc[i,'charLengthVarW']);  
        charPatchWVarParamVar[i] = float(dfinput.loc[i,'charLengthVarWVAR']);
    ...
    #randomly select graphene patch in a for loop for N locations
        ...
        for h in range(layers, -1, -1): #do this for each layer
            ...
            charPatchLVar = abs(np.random.normal([charPatchLVarParam[h]], abs(charPatchLVarParamVar[h])));        # characteristic patch length variance
            charPatchL = abs(np.random.normal(charPatchLParam[h], abs(charPatchLParamVar[h])));# mean characteristic patch length
            charPatchWVar = abs(np.random.normal(charPatchWVarParam[h], abs(charPatchWVarParamVar[h])));
            charPatchW = abs(np.random.normal(charPatchWParam[h], abs(charPatchWParamVar[h])));# mean patch width
