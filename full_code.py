...
#randomly select graphene patch in a for loop for N locations
    ...
    for h in range(layers, -1, -1): #do this for each layer
        ...
        coverageFlag=0;
        timenow=0
        while(coverage[h]<desiredCoverage[h]):
            overlapFlag = True;
...
def coverageCheck(dfEx, dfMod):#import experimental and mod array, return an array of pathcoverage
    ...
    for i in range(numModels):
        ...
        distCoverage[i] = np.sum(np.abs(np.subtract(dfModSorted[i].to_numpy(), dfExSorted.to_numpy())))
    return distCoverage
...
def charLengthCheck(r, layer, dfEx, dfMod): #input: r value, layer; output: charLengthW, charLengthL, charLengthWVar, charLengthLVar 
    ...
    dfEx_grayScale[dfEx_grayScale > maxVal] = 0
    dfEx_grayScale[dfEx_grayScale < minVal] = 0
    ...
    for i in range(numModels):
        ...
        dfMod_grayScale[i][dfMod_grayScale[i] > maxVal] = 0
        dfMod_grayScale[i][dfMod_grayScale[i] < minVal] = 0
    ...
    for i in range(numModels):
        ...
        dfMod_charLength[i,1] = abs(dfMod_charLength[i,1]-lengthDistanceEx)
        dfMod_charLength[i,2] = abs(dfMod_charLength[i,2]-widthDistanceEx)
        dfMod_charLength[i,3] = abs(dfMod_charLength[i,3]-lenVarDistanceEx)
        dfMod_charLength[i,4] = abs(dfMod_charLength[i,4]-widVarDistanceEx)
        
    return dfMod_charLength

#test function for one layer
layer0 = charLengthCheck(0.5,1,dfEx,dfMod);
